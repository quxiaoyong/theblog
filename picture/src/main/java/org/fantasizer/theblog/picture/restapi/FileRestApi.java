...
import org.fantasizer.theblog.picture.global.SQLConfiguration;
import org.fantasizer.theblog.picture.global.SystemConfiguration;
import org.fantasizer.theblog.picture.service.FileCatalogService;
...
public class FileRestApi {
    ...
    public String hello(String urlString, int i) throws IOException {
        ...
        try {
            ...
            return ResultUtil.result(SystemConfiguration.ERROR, "无法获取头像");
        }
    ...
    public String getPicture(
        ...
        if (StringUtils.isEmpty(fileIds)) {
            ...
            QueryWrapper<org.fantasizer.theblog.picture.entity.File> queryWrapper = new QueryWrapper<>();
            queryWrapper.in(SQLConfiguration.UID, changeStringToString);
            List<org.fantasizer.theblog.picture.entity.File> fileList = fileService.list(queryWrapper);
            if (fileList.size() > 0) {
                for (org.fantasizer.theblog.picture.entity.File file : fileList) {
                    if (file != null) {
    ...
    public Object downloadFile(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isEmpty(fileId)) {
            ...
            org.fantasizer.theblog.picture.entity.File oneById = fileService.getById(fileId);
            if (oneById != null) {
                ...
                try {
                    ...
                    return ResultUtil.result(SystemConfiguration.SUCCESS, null);
                } catch (Exception e) {
                    e.printStackTrace();
                    log.info("============文件下载出现异常==========");
                    return ResultUtil.result(SystemConfiguration.ERROR, "文件下载出现异常");
                } finally {
                    if (bis != null) {
                        try {
                            bis.close();
        ...
        return ResultUtil.result(SystemConfiguration.ERROR, "参数错误");
    }
    ...
    public synchronized Object uploadPics(HttpServletResponse response, HttpServletRequest request, List<MultipartFile> filedatas) {
        ...
        if (StringUtils.isEmpty(userUid) && StringUtils.isEmpty(adminUid)) {
            return ResultUtil.result(SystemConfiguration.ERROR, "请先注册");
        } else {
        ...
        queryWrapper.eq(SQLConfiguration.SORT_NAME, sortName);
        queryWrapper.eq(SQLConfiguration.PROJECT_NAME, projectName);
        queryWrapper.eq(SQLConfiguration.STATUS, Status.ENABLED);
        List<FileCatalog> fileSorts = fileCatalogService.list(queryWrapper);

        System.out.println("fileSorts" + JsonHelper.objectToJson(fileSorts));

        FileCatalog fileSort = null;
        if (fileSorts.size() > 0) {
            fileSort = fileSorts.get(0);
            log.info("====fileSort====" + JsonHelper.objectToJson(fileSort));
        } else {
            return ResultUtil.result(SystemConfiguration.ERROR, "文件不被允许上传");
        }
        ...
        List<org.fantasizer.theblog.picture.entity.File> lists = new ArrayList<>();
        //文件上传
        if (filedatas != null && filedatas.size() > 0) {
            for (MultipartFile filedata : filedatas) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    return ResultUtil.result(SystemConfiguration.ERROR, "文件上传失败");
                }
                org.fantasizer.theblog.picture.entity.File file = new org.fantasizer.theblog.picture.entity.File();
            ...
            //保存成功返回数据
            return ResultUtil.result(SystemConfiguration.SUCCESS, lists);
        }
        return ResultUtil.result(SystemConfiguration.ERROR, "请上传图片");
    }

    @ApiOperation(value = "通过URL上传图片", notes = "通过URL上传图片")
    @PostMapping("/uploadPicsByUrl")
    public synchronized Object uploadPicsByUrl(@Validated({FetchList.class}) @RequestBody FileVO fileVO, BindingResult result) {
